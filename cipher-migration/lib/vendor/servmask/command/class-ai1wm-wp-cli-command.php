...
if ( class_exists( 'WP_CLI_Command' ) ) {
	class Ai1wm_WP_CLI_Command extends WP_CLI_Command {
		 ...
		 *
		 * Example:
		 * $ wp ncres restore migration-wp-20170913-095743-931.ncpress
		 * Restore in progress...
		 * Restore complete.
		 *
		 * @subcommand restore
		...
		public function restore( array $args, array $assoc_args ) {
			...
			if ( isset( $args[0] ) && is_file( ai1wm_backup_path( array( 'archive' => $args[0] ) ) ) ) {
				...
				WP_CLI::error_multi_line( array(
					...
					__( 'Example: wp ncres restore migration-wp-20170913-095743-931.ncpress', AI1WM_PLUGIN_NAME ),
				) );
				exit;
			}
...
if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'ncres', 'Ai1wm_WP_CLI_Command', array( 'short_desc' => 'Cipher Migration Command' ) ); // CIPHER_REBRAND
WP_CLI::add_command( 'ai1wm', 'Ai1wm_WP_CLI_Command', array( 'short_desc' => 'Cipher Migration Command (legacy alias)' ) );
}
